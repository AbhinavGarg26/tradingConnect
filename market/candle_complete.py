...
class CandleCompletionScheduler:
    ...
    def check_and_sync(self, kite, db, symbol: str, token: int):
        ...
        # -------------------------------------------------------------
        if minute % 15 == 0 and self.last_triggered_15m != current_minute_str:
        ...
        if is_1h_boundary and self.last_triggered_1h != current_minute_str:
        ...
        # Standard NSE 3H candles end at: 12:15, 15:15
        # -------------------------------------------------------------
        is_3h_boundary = (minute == 15 and hour in [12, 15])
        if is_3h_boundary and self.last_triggered_3h != current_minute_str:
